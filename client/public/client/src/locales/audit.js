...
function parseTsToJsObject(filePath) {
  ...
  // Simple regex to extract the object part. This might be brittle, let's see.
  // Actually, we can just compile it to JS and require it.
  const result = ts.transpileModule(content, {
    compilerOptions: { module: ts.ModuleKind.CommonJS }
  });
  
  const tempFile = filePath.replace('.ts', '_temp.js');
...
try {
  ...
  function compareObjects(obj1, obj2, path = '', obj1Name = 'en', obj2Name = 'nl') {
    ...
    keys1.forEach(key => {
      ...
      if (!keys2.includes(key)) {
        if (obj1Name === 'en') missingInNl.push(currentPath);
        else missingInEn.push(currentPath);
      } else {
        if (typeof obj1[key] === 'object' && obj1[key] !== null) {
          compareObjects(obj1[key], obj2[key], currentPath, obj1Name, obj2Name);
        }
      }
    });
  }
